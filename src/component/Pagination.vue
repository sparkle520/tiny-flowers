...
const init = () => {
  ...
  nextTick(() => {
    let last_number = parseInt(
      current_index.value
        .toString()
        .charAt(current_index.value.toString().length - 1)
    );
...
const current_index = ref(props.data.current_index);
watch(current_index, (newV, oldV) => {
  emit("page_change", current_index.value);
...
const page_handle = (page_index) => {
  current_index.value = page_index;
};
