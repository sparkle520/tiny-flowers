...
<template>
  <div id="math_main" class="f  r j_c_c">
    <svg class="fixed bg_shape" xmlns="http://www.w3.org/2000/svg" version="1.1"
    ...
    <div>
      <div id="top_box" class="f f_d_c com_box a_c r">
        ...
        <div class="f f_d_c select_tag">
          <h4 class="">标签：</h4>
          <ul class="f f_d_r r">
            <div @click="show_all_tag" v-show="!show_all_tag_flag" class="a show_all_tag f f_d_r a_c">
              展开全部<svg t="1708943924609" class="icon" viewBox="0 0 1024 1024" version="1.1"
                xmlns="http://www.w3.org/2000/svg" p-id="10332" width="18" height="18">
                <path
                  ...
                  fill="#A9B8C0" p-id="10333"></path>
              </svg>
            </div>
            <li @click="router.push(`/math/${item[0]}/1`)" v-for="item in tags_list" :key="item" class="r">
              {{ item[0] }}
              <span class="count"> {{ item[1] }} </span>
            </li>
            <div @click="close_all_tag" v-show="show_all_tag_flag" class="close_all_tag f f_d_r a_c">
              收起<svg t="1708943924609" class="icon" viewBox="0 0 1024 1024" version="1.1"
                xmlns="http://www.w3.org/2000/svg" p-id="10332" width="18" height="18">
                <path
                  d="M737.701647 149.865412l47.104 37.647059v0.12047L520.372706 519.830588 255.879529 187.572706l46.98353-37.707294 217.449412 273.829647 217.389176-273.829647zM520.131765 785.106824l217.328941-273.829648 47.104 37.707295-264.432941 332.257882-264.493177-332.257882 46.98353-37.707295 217.509647 273.829648z"
                  fill="#A9B8C0" p-id="10333"></path>
      ...
      <div id="content_box" class="f f_d_c a_c">
        <!-- width：100% -->
        <ul class="content_mid_box f f_d_c">
          <li v-for="item in current_data.slice(0, 10)" :key="item" class="content_item_box fade_out f f_d_c">
            <div class="f f_d_r content_item_top_box">
              ID:{{ item.id }}<span>{{ item.from }}</span>
            </div>

            <div @click="item.show_solution = !item.show_solution" class="subject_context markdown-body"></div>
            <div v-show="item.show_solution" class="subject_solution_context markdown-body"></div>
            <div class="content_tag_box f a_c">
              <div v-for="item2 in item.tags.split('?')" class="f content_tag_item a_c j_c_c" :key="item2">
                {{ item2 }}
              </div>
            </div>
            <div class="content_item_bottom_box f f_d_r">
              <button @click="router.push(`/math/details/${item.id}`)" class="content_item_bottom_btn">
                查看详情
              </button>
            </div>
          </li>
        </ul>
        <div class="content_bottom_box f a_c j_c_c">
          <Pagination style="z-index: 100" :current_page="current_page" page_size="10"
            @current_change="current_page_change" :total="math_store.current_data.data.length">
          </Pagination>
        </div>
      </div>
    </div>
  ...
  <div class="math_foot r f f_d_r a_c j_c_c">
    ...
    .content_mid_box {
      ...
      .content_item_box {
        ...
        .subject_solution_context {
          ...
          margin: 8px 16px;
          padding: 16px;
          background-color: color-mix(in srgb, $subject_solution_context_bg 90%, $primary_mix_3 10%) !important;
        }
