...
1. 交换律：$A\\cup B=B\\cup A$，$AB=BA$\r
1. 结合律：$(A\\cup B)\\cup C=A\\cup (B\\cup C)$，$ABC=(AB)C=A(BC)$\r
1. 分配律：$A\\cup (BC)=(A\\cup B)(A\\cup C)$，$A(B\\cup C)=AB\\cup AC$，$A(B - C) =AB - AC$\r
1. **对偶律**：$\\overline{A\\cup B}=\\overline{A}\\cap \\overline{B}$，$\\overline{A\\cap B}=\\overline{A}\\cup \\overline{B}$\r
1. 事件的积：$A\\cap B=AB$\r
1. 事件的和：$A\\cup B=A+B$ ($A,B$互不相容)\r
1. 事件的差：$A-B=A\\overline{B}$\r
\r
## 概率和条件概率的概念\r
\r
...
\r
**【例】**：抛掷两枚骰子,在第一枚骰子出现的点数能够被 3 整除的条件下,求两枚骰子出现的点数之和大于 8 的概率.\r
\r
**【解】**： 设 A 表示事件“第一枚骰子出现的点数能够被 3 整除”,B 表示事件“两枚骰子出现的点数之和大于 8”。抛掷两枚骰子所出现的点数为$(i,j)(i,j=1,2,\\cdots,6)$,其中$i,j$分别表示抛掷第一枚骰子和抛掷第二枚骰子出现的点数,共有$6^2$=36 种结果,即有 36 个基本事件。抛掷第一枚骰子出现 3 点或 6 点时,才能被 3 整除,因此事件 A 包含 2 个基本事件,从而\r
$$P(A)=\\frac{2}{6}=\\frac{1}{3}.$$\r
事件 A 和事件 B 的交 AB=$\\left\\\\{ (3,6), (6,3), (6,4), (6,5), (6,6) \\right\\\\}$，即包含 5 个基本事件，因此\r
$$P(AB)=\\frac{5}{36}.$$\r
因此，所求概率即为条件概率\r
$$P(B|A)=\\frac{P(AB)}{P(A)}=\\frac{\\frac{5}{36}}{\\frac{1}{3}}=\\frac{5}{12}.$$\r
\r
**【例】**：在一个盒子中放有 10 个乒乓球,其中 8 个是新球,2 个是用过的球.在第一次比赛时,从该盒子中任取 2 个乒乓球,比赛后仍放回盒子中.在第二次比赛时从这个盒子中任取 3 个乒乓球,则第二次取出的都是新球的概率为\\_\\_\\_\\_.\r
\r
**【解】**：设$A_i$表示事件“在第一次比赛时取出的 2 个球中有 $i$ 个是新球,其余是用过的球”$(i=0.1.2)$,$B$ 表示事件“在第二次比赛时取出的球全是新球”,则有\r
...
$$P(B)=\\sum_{i=0}^2 P(A_i)P(B|A_i)=\\frac{1}{45} \\cdot \\frac{7}{15}+\\frac{16}{45} \\cdot \\frac{7}{24}+\\frac{28}{45} \\cdot \\frac{1}{6}=\\frac{49}{225} \\approx 0.218.$$\r
\r
## 事件的独立性\r
\r
### 定义\r
...
1. $$\r
   \\begin{align}\r
   A,B 相互独立 & \\Leftrightarrow P(AB)=P(A)P(B)\\\\\\\\\r
